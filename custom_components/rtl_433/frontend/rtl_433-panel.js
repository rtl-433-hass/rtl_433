 ...
 *
 * The overview describes the *location*, not one radio: its counts and its
 * links cover every receiver in it, so the card above them has to as well. One
 * receiver online and a second one down is the case that makes this matter --
 * a card that read "Online" there would be reporting the location's merged
 * connection as though every radio behind it were up.
 *
 ...
 *
 * Filtered by the *location's* config entry, which is the same set the row's
 * own count is taken from: a receiver is a subentry, so one entry filter covers
 * every radio in the location and nothing from another one. `config_entry` is
 * the filter Home Assistant's own devices and entities dashboards read out of
 * the URL, and `historyBack=1` is what makes their back arrow return to this
 * panel instead of to their own tab.
 *
 * Exported so the filter is pinned by a test: a link whose scope disagreed with
 * the count beside it was the bug, and the difference between the two URLs is
 * one word on screen nowhere.
 */
export function configPagePath(which, entryId) {
  return `/config/${which}/dashboard?historyBack=1&config_entry=${entryId}`;
}
...
class Rtl433Panel extends HTMLElement {
   ...
   *
   * Every receiver of the location, not the subscribed one's: a receiver is a
   * subentry of the location, so the location's entry filter already covers all
   * of them. The count beside the link is counted the same way -- off the same
   * entry -- so what a user reads is what they land on.
   */
  _openConfigPage(which) {
    if (!this._entryId) {
      return;
    }
    const path = configPagePath(which, this._entryId);
    window.history.pushState(null, "", path);
    window.dispatchEvent(
  ...
  /**
   * Every receiver of this location, as one connection state each.
   *
   * Deliberately not the payload's merged `connected`: that one is true while
   * *any* receiver of the location is up, which is the right thing to union
   * entities by and the wrong thing to put on a card. One receiver online and a
   * second one down is the case that makes this matter -- a card reading
   * "Online" there would be answering a question nobody asked.
   *
   * The rows come from `rtl_433/receivers`, which the clock re-asks for while
   * the overview is on screen (see `_tick`), so they cannot sit on a stale
   * "Online". A row with no `connected` yet reports `null` -- not known is not
   * "offline", and a card that flashed a problem on the way in would cry wolf
   * on every visit.
   */
  _receiverConnections() {
    return this._receiversOf(this._entryId).map((receiver) =>
      typeof receiver.connected === "boolean" ? receiver.connected : null
    );
  }
