...
# default, so once healed an entry cannot re-acquire it. Equal to
# ``DEFAULT_AVAILABILITY_TIMEOUT`` today, but pinned separately so the migrations
# stay correct if that default ever changes.
