...
class Rtl433Coordinator(_SdrSettingsMixin, _EventProcessingMixin, _AvailabilityMixin):
    ...
    def forget_device(self, device_key: str) -> None:
        ...
        silently re-creating the device the user just deleted. Its runtime state
        goes with it, so nothing in ``devices`` / ``last_seen`` / ``available`` /
        ``device_fields`` describes a device that no longer exists in Home
        Assistant.
