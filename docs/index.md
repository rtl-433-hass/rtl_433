...
![A single rtl_433 receiver in Home Assistant with its nested devices: a weather sensor, energy meter, door sensor, doorbell, leak detector, and utility meter](images/09-home-hero.png)
...
- **Configurable availability**: silence-based availability with receiver defaults,
  device overrides, and event-driven class defaults.
- **Multiple servers**: add one receiver per rtl_433 server; device identities are
  scoped per receiver.
- **Receiver observability**: diagnostic entities for connectivity, SDR/meta values,
...
- Add a receiver with [Configuration](configuration.md).
