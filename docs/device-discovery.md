...
Open **Settings → Devices & Services → rtl_433 → Configure (the gear icon)**. This is the
...
If a location has more than one receiver, the top of the page covers all of
them: the device count and both **My network** links include every device in the
location, whichever receiver heard it, and the card at the top says how many
receivers are offline rather than **Online** when one of them is down.
