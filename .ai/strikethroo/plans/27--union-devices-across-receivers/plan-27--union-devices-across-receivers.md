...
| 20 | What happens when a user deletes a receiver subentry from a location? | **Remove that receiver's own entities; keep merged devices** *(user-selected)*. Drop the receiver device, its radio controls, noise sensors, connectivity entity, and its per-receiver diagnostic entities on every merged device; **merged sensor devices and their history survive**, and merged availability is recomputed over the remaining receivers. A merged device that only the removed receiver ever heard goes **unavailable but is not deleted** — deleting it stays an explicit user action. |
...
These diagnostics are the one place where the subentry-ownership rule is genuinely counter-intuitive and must be called out (Clarification #17): they are *about* a receiver but they *hang off the merged device*, so they must be added with **no** `config_subentry_id`. Adding them under their receiver's subentry would make entities from two subentries share one device, which silently moves the merged device today and **raises in HA Core 2027.8**. The receiver association is carried by the entity's name, translation key and unique_id segment — never by its subentry.

**Removing a receiver** (Clarification #20) is the inverse operation and is specified here rather than left to the registry's default cascade: deleting a receiver subentry removes that receiver's device, its radio controls, noise sensors and connectivity entity, **and its per-receiver diagnostic entities on every merged device**; merged sensor devices and their history **survive**, and merged availability is recomputed over the remaining receivers. A merged device that only the removed receiver ever heard therefore goes **unavailable but is not deleted** — removing it stays an explicit user action, consistent with the existing device-removal flow.
...
- **Merged device silently moved between subentries, raising in HA Core 2027.8**: a device belongs to exactly one subentry, and adding entities from two subentries that share a device silently moves it (`device_registry.py:2482-2500`). Component 5's per-receiver diagnostics are precisely this shape — receiver-specific entities on a shared merged device — so the natural implementation is the broken one, and it fails as a log line today and a hard error later.
    - **Mitigation**: Fix the ownership rule at every `async_add_entities` call site (Clarification #17): merged-device entities, diagnostics included, pass **no** `config_subentry_id`; only receiver-owned entities pass one. Test that the merged device's `config_subentry_id is None` with two receivers attached, and assert no deprecation warning is emitted (`caplog`) after both receivers' platforms have loaded.
...
14. **The 4-segment templates survive both segment parsers**: `device_trigger.py`'s `_device_field_from_unique_id` and `__init__.py`'s orphan-device scan each correctly classify all three unique_id shapes, never decoding a receiver control or diagnostic as a device field, and `receiver` is enforced as a reserved token no `device_key` may equal.
15. **Setting up the integration is a single step**: `async_step_user` collects host/port/path as today and creates the location entry plus its first receiver subentry together; no receiver-less location is reachable.
16. **Removing a receiver is non-destructive to merged data**: deleting a receiver subentry removes its own device and entities and its per-receiver diagnostics from every merged device, while merged devices keep their `entity_id`s and history; a device still heard by another receiver stays available, and one heard only by the removed receiver becomes unavailable rather than being deleted.
17. The full unit test suite passes, including new tests for cross-receiver union, timestamp dedup, merged availability (both gates), the union pending list, the subentry flow and ownership, the unique-id parsers, the control unique-id migration, receiver removal, and the duplicate-history auto-merge.
...
| Per-receiver diagnostic entities (on the merged device) | location entry | **`None`** — receiver identity is carried by the unique_id and name, never by the subentry |
...
graph TD
    ...
    T5["Task 5: Merged availability + diagnostics + receiver removal"]
...
**Parallel Tasks:**
- Task 003: Location-scoped device identity (merged device owned by the location entry, `config_subentry_id=None`) and `via_device_id` relink (depends on: 002)
...
**Parallel Tasks:**
- Task 005: Merged two-gate availability, per-receiver diagnostics (added with no subentry id), and receiver-removal semantics (Clarification #20) (depends on: 004)
...
- Task 012: Docs & screenshots, including the `device_replace.py` module docstring whose hardcoded v2 templates this plan falsifies (depends on: 009)
