...
All 12 tasks across 7 phases completed; the working tree is clean and every task landed as its own conventional commit on `main` (per the plan's one-commit-per-task cadence). Delivered a complete, HACS-installable `custom_components/rtl_433` Home Assistant integration:

- **Tooling & CI** (T01–T02): `pyproject.toml` (ruff lint+format), split requirements, pre-commit, Renovate, release-please, `hacs.json`, and GitHub Actions (lint, hassfest+HACS validate, pytest matrix, conventional-commits, CodeQL, release) — to be verified on the user-provided remote.
- **Integration** (T03, T05–T09): package skeleton + shared `const.py`; a data-driven device-mapping library (10 themed YAML files, 59 field descriptors + 15 skip keys, ported from `rtl_433_mqtt_hass.py`) with a thin loader and user-override support; a deterministic event normalizer; a push WebSocket coordinator (reconnect/backoff, keep-alive tolerance, dispatcher, availability watchdog); hub + per-device + discovery + options config flows (Battery-Notes style, `SOURCE_IGNORE`, instance-scoped unique_ids, `ws://`/`wss://`, no in-integration auth); `sensor`/`binary_sensor` platforms with dynamic late-field creation, observed-field persistence, `RestoreEntity` restore, and availability wiring; full lifecycle wiring with hub-deletion cascade removal, redacted diagnostics (with unmatched-field-key detection), and a minimal server-unreachable repair.
- **Device library** (T04): themed YAML + contributor guide (`docs/device-library.md`).
- **Tests** (T10): 34 passing tests (`pytest-homeassistant-custom-component`), ~89% coverage of `custom_components/rtl_433`, driving project-authored JSON fixtures through mapping, normalization, coordinator, flows, and the full lifecycle.
- **Containerized harness** (T11): pinned `hertzg/rtl_433` + Home Assistant Docker images replaying real captures from the pinned, sparse `merbanan/rtl_433_tests` submodule, with Playwright capturing four real screenshots (discovery card, device page, options flow, unavailable state) committed under `docs/images/`. The live run brought up all containers, the integration connected and created a real `Acurite-Tower` device with correctly-classified entities, and the availability watchdog was observed flipping unavailable then recovering.
- **Docs** (T12): `README.md`, `AGENTS.md`, `CONTRIBUTING.md` (markdownlint clean).

**Self Validation coverage:** the connection/discovery/device-page/options/unavailable bullets were verified *live* against real Home Assistant via the T11 container harness (with committed screenshot evidence). The remaining bullets — ignore prevents re-prompt, multi-instance distinct unique_ids, user-override YAML adds an entity, hub cascade removal leaves no orphans, intermittent late-field creation persisting across restart, and restore-then-timeout — are covered by the passing unit suite (T10). The 34-minute live harness was not re-run by the coordinator since the T11 evidence is conclusive.

### Noteworthy Events

- **Greenfield repo, upstream seed fetched remotely:** the repo contained only scaffolding; the `rtl_433_mqtt_hass.py` mappings seed was fetched from upstream GitHub by the T04 agent (not present locally).
- **Coordinator-managed commits:** to keep file-disjoint parallel agents from racing on `.git/index.lock` and `pip`, sub-agents performed file work + local validation only, and the coordinator made each per-task conventional commit sequentially (staging only that task's files). Net effect still matches the one-commit-per-task cadence.
- **Shared venv:** the runtime lacked `python3-venv`; installed it via passwordless sudo, then pre-built a shared venv (Home Assistant 2026.2.3, ruff, `pytest-homeassistant-custom-component`) so all agents validated against real HA without per-agent installs.
- **Feature-branch step skipped** (Clarification #11): all work landed directly on `main`.
- **Phase 6 sequenced, not parallel:** although the blueprint scheduled T10 and T11 in parallel, the coordinator ran T10 first (validate/harden the integration) then T11 (which installs the integration into a real HA container), since T11's correctness depends on validated code. No bug was found by T10 (the code passed as written).
- **Clarification #13 falsified during execution:** `rtl_433 -r cu8:<fifo> -F http` does **not** stream over WebSocket — file/`-r` input runs in test mode and exits before rtl_433 starts its mongoose HTTP/WS loop (verified against rtl_433 v25.12). The harness keeps the planned FIFO keep-alive but emits `-F json` and adds a small Node ws-bridge that re-broadcasts the JSON on `ws://:8433/ws` (the exact frame shape the coordinator expects). This is a harness-only transport stand-in; the shipped integration is unchanged. Documented in `tests/integration/README.md`.
- **Real bug caught by the live harness and fixed:** HA flagged a blocking `open()` on the event loop — the entity platforms called `mapping.lookup()` without a registry, triggering the lazy synchronous YAML load during entity setup. Fixed by passing the hub's already-cached (executor-loaded) merged registry; `DATA_LIBRARY` was promoted into `const.py` to remove a magic-string duplication, and a regression test was added (commit `d29aa89`).
- **`battery_ok` modeling:** ported faithfully from upstream as a **percentage sensor** (`device_class: battery`, `0→1%`, `1→100%`) rather than the binary low-battery example sketched in the task; the binary-problem alternative is documented as a user-override example.
- **PyYAML boolean keys:** unquoted `on`/`off` keys in the binary `payload` YAML parse as Python `True`/`False`; the loader normalizes them back to canonical string keys.
- **Transient API error:** the first documentation agent's connection dropped before writing files; a relaunch completed cleanly.
- **Screenshot placement:** the four gallery screenshots were relocated from the gitignored `screenshots/` (the harness's ephemeral output dir) to `docs/images/` so the committed README gallery lives in a clean, non-ignored location.
- **Cleanup:** removed internal "Task N" references from shipped source comments (they would dangle after archival), rewriting them to name the actual modules (commit `8330b52`).

### Necessary follow-ups

- **Remote CI verification (Success Criteria #7–#8):** push to the user-provided GitHub remote so hassfest + HACS validation, lint, conventional-commit, and the pytest matrix run there (not runnable locally; the standalone hassfest needs HA-core's `script/`).
- **Add a `LICENSE` file:** none exists. HACS default-repo inclusion requires one, and the README references none. Out of the plan's stated scope but worth adding before publishing.
- **Native rtl_433 `-F http` replay path:** the harness uses a ws-bridge stand-in because file/FIFO replay can't drive rtl_433's HTTP server; for a fully native path, replay through a live `rtl_tcp` source. Documented in `tests/integration/README.md`.
- **Live-verify the unit-tested Self Validation bullets** (ignore re-prompt, multi-instance, user override, cascade, late-field-across-restart, restore-then-timeout) through the container harness when convenient, to complement the existing unit coverage.
- **Translations breadth:** only `en.json` ships (config/options/issues). Add further languages as the user base grows.
