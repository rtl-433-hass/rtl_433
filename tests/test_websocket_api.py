...
async def test_receivers_lists_every_configured_receiver_loaded_or_not(
    ...
    # receiver it is opened against. The panel's overview describes *every* receiver
    ...
    assert by_id[receiver.entry_id]["connected"] is True
    assert by_id[unloaded.entry_id]["connected"] is False


@pytest.mark.receiver_disconnected
async def test_receivers_report_a_loaded_receiver_with_no_socket_as_disconnected(
    hass, receiver, hass_ws_client
):
