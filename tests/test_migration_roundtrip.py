...
def _device_identifier_map(dev_reg: dr.DeviceRegistry) -> dict[tuple[str, str], str]:
    ...
    for device in dev_reg.devices:
        for ident in device.identifiers:
            if ident[0] == DOMAIN:
                # Identifiers are unique per config entry, not globally, so two
                # devices *can* share one; capturing device_id is what lets the
                # test assert this integration never splits one that way.
